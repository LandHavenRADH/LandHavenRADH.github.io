...
import { Deal, Task, Contact, Franchise, LibraryItem } from './types';
...
export default function App() {
  ...
  const openModal = (name: keyof typeof modals, params: any = {}) => {
    setModals(prev => ({ ...prev, [name]: { ...prev[name], open: true, ...params } }));
  };
          ...
          <div className="p-8">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="name@company.com" required />
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Password</label>
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all" placeholder="••••••••" required />
              </div>
    ...
    <div className="bg-slate-100 text-slate-900 h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden relative">
        ...
        <main className="flex-1 relative bg-slate-100 overflow-hidden">
          ...
          {currentView === 'franchises' && <FranchisesView franchises={franchises} openModal={openModal} />}
