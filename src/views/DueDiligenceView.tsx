...
export default function DueDiligenceView({ deals, franchises, openModal }: { deals: Deal[], franchises: Franchise[], openModal: (name: string, params?: any) => void }) {
  
  const moveDDStage = async (id: string, ddStatus: string) => {
    await updateDoc(doc(db, 'gc_deals', id), { ddStatus });
  };
  ...
  return (
    <div className="absolute inset-0 p-8 flex flex-col h-full">
      <div className="flex justify-between items-center mb-6 shrink-0">
        <div>
          <h2 className="text-2xl font-bold text-slate-800">Due Diligence Tracking</h2>
          <p className="text-slate-500 text-sm">Manage environmental, survey, and title action items per deal.</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden pb-4">
        <div className="flex gap-6 min-w-max h-full">
          {DD_STAGES.map(stage => {
            const stageDeals = deals.filter(d => (d.ddStatus || 'not_started') === stage.id && d.stage !== 'cancelled' && d.stage !== 'sold');
            return (
              <div key={stage.id} className="w-80 flex flex-col h-full shrink-0">
                ...
                <div className="flex-1 bg-slate-200/50 rounded-lg p-2 space-y-3 overflow-y-auto no-scrollbar">
                  {stageDeals.map(deal => {
                    const totalTasks = deal.checklist ? deal.checklist.length : 0;
                    const completedTasks = deal.checklist ? deal.checklist.filter(t => t.completed).length : 0;
                    ...
                    return (
                      <div key={deal.id} className="bg-white p-4 rounded-lg shadow-sm border border-slate-200 hover:shadow-md transition group cursor-pointer relative" onClick={() => openModal('dd', { id: deal.id })}>
                        ...
                        <div className="flex justify-between items-center mt-2 pt-2 border-t border-slate-100">
                          <span className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">DD Status</span>
                          <select onClick={e => e.stopPropagation()} onChange={e => moveDDStage(deal.id, e.target.value)} value={deal.ddStatus || 'not_started'} className="text-xs bg-slate-100 border-none rounded px-2 py-1 cursor-pointer hover:bg-slate-200 focus:ring-0 font-medium text-slate-700">
                            {DD_STAGES.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}
                          </select>
                        </div>
                      </div>
                    );
