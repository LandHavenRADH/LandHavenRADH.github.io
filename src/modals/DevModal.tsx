...
import { X, ExternalLink, HardHat, User, Link as LinkIcon, DollarSign, Plus, UserPlus, Trash2, Check } from 'lucide-react';
import { Deal } from '../types';
import { doc, updateDoc } from 'firebase/firestore';
import { db } from '../firebase';

export default function DevModal({ closeModal, dealId, deals, openModal, setCurrentView }: { closeModal: () => void, dealId: string | null, deals: Deal[], openModal: (name: string, params?: any) => void, setCurrentView: (v: string) => void }) {
  ...
  const checklist = deal.devChecklist || [];
  ...
  const toggleItem = async (itemId: string, completed: boolean) => {
    ...
    await updateDoc(doc(db, 'gc_deals', dealId), { devChecklist: updated });
  };
  ...
  const addItem = async () => {
    ...
    const newItem = { id: Date.now().toString() + 'd-' + Math.random().toString(36).substring(2, 5), text: newItemText, completed: false };
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-4 border-b bg-slate-50 shrink-0">
          <div>
            ...
            <p className="text-sm text-slate-500">Site Development Checklist</p>
          </div>
          ...
          <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm">
            <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-2">
              <h4 className="font-bold text-slate-800 flex items-center gap-2">
                <HardHat className="text-amber-600" size={18} /> Pad Ready Items
              </h4>
              <div className="text-xs font-bold text-slate-600 bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200 shadow-sm">
                Total Dev Cost: <span className="text-amber-600">{totalCostStr}</span>
              </div>
            </div>
            
            <div className="mb-4">
              {checklist.length === 0 ? (
                <div className="text-slate-400 text-sm italic py-1">No construction items. Add one below.</div>
              ) : (
                checklist.map(item => {
                  let urls = item.documentUrls || [];
                  if (item.documentUrl && urls.length === 0) urls = [{ name: 'Document', url: item.documentUrl }];
                  ...
                  return (
                    <div key={item.id} className="flex flex-col group bg-white p-3 rounded-lg border border-slate-100 hover:border-amber-200 transition-colors shadow-sm mb-2 gap-2">
                      <div className="flex items-center justify-between gap-2">
                        <label className="flex items-center gap-3 cursor-pointer flex-1 min-w-0 pr-2">
                          <input type="checkbox" checked={item.completed} onChange={e => toggleItem(item.id, e.target.checked)} className="w-5 h-5 rounded border-slate-300 text-amber-600 focus:ring-amber-500 cursor-pointer shrink-0" />
                      ...
                      <div className="flex flex-wrap items-center gap-2 pl-8">
                        {item.contactName ? (
                          ...
                          <button onClick={(e) => { e.stopPropagation(); openModal('contact', { dealId, devItemId: item.id }); }} className="text-xs text-blue-600 bg-blue-50 hover:bg-blue-100 px-2 py-1.5 rounded-md border border-blue-200 transition flex items-center gap-1 opacity-0 group-hover:opacity-100 whitespace-nowrap shrink-0">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity mb-1 flex-wrap" onClick={e => e.stopPropagation()}>
                          <input type="text" id={`dev-link-name-${item.id}`} placeholder="File Name..." className="text-xs p-1.5 border border-slate-200 rounded outline-none focus:border-purple-500 w-20" />
                          <input type="url" id={`dev-link-url-${item.id}`} placeholder="Drive URL..." className="text-xs p-1.5 border border-slate-200 rounded outline-none focus:border-purple-500 w-32" 
                            onKeyPress={(e) => {
                              if (e.key === 'Enter') {
                                const nameInput = document.getElementById(`dev-link-name-${item.id}`) as HTMLInputElement;
                          ...
                          <button onClick={() => {
                              const nameInput = document.getElementById(`dev-link-name-${item.id}`) as HTMLInputElement;
                              const urlInput = document.getElementById(`dev-link-url-${item.id}`) as HTMLInputElement;
                              assignLink(item.id, nameInput.value, urlInput.value);
                              nameInput.value = '';
                              urlInput.value = '';
                        ...
                        {item.cost ? (
                          ...
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity mb-1" onClick={e => e.stopPropagation()}>
                            ...
                            <input type="text" id={`dev-cost-${item.id}`} placeholder="Cost..." className="text-xs p-1.5 border border-slate-200 rounded outline-none focus:border-amber-500 w-20" 
                              onKeyPress={(e) => {
                                if (e.key === 'Enter') {
                            ...
                            />
                            <button onClick={() => {
                                const input = document.getElementById(`dev-cost-${item.id}`) as HTMLInputElement;
            ...
            <div className="flex gap-2 pt-3 border-t border-slate-100">
              <input type="text" value={newItemText} onChange={e => setNewItemText(e.target.value)} onKeyPress={e => e.key === 'Enter' && addItem()} className="flex-1 p-2 border border-slate-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-amber-500" placeholder="Add custom construction task..." />
